...
Codex는 공용 하네스의 `$do-next` 흐름을 따른다.
...
- 의도와 결정 경계는 Codex 환경에 설치된 `$deep-interview`로 좁힌다.
- Codex App에서 `omx question`을 쓸 수 없으면 네이티브 structured input을 사용한다.
- structured input도 없으면 한 번에 하나의 명시 질문만 묻는다.
