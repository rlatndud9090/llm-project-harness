...
넓게 시작할 때 사용하던 호환 프로토콜이다. 새 표준 진입점은 `$do-next`이며,
이 문서는 `$do-next` 내부의 후보 발굴 단계로 사용한다. 이 프로토콜은 구현하지
않는다. 목표는 현재 프로젝트 상태와 사용자 의도를 바탕으로 작고 검증 가능한
작업 단위 후보를 만드는 것이다.
...
새 작업에서는 이 역할들을 직접 호출하기보다 `$do-next`가 오케스트레이션한다.
...
- **나쁨:** 후보를 만들자마자 구현한다.
- **좋음:** `$do-next`가 후보 승인, branch/raw 생성, PRD/ADR 승인까지 진행하고
  구현은 별도 요청으로 넘긴다.
