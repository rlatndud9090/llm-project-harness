...
`<프로젝트 약어> one-shot` 형식으로 바꾼다(대화형·background 세션 모두 항상). 어느 세션이
무인 파이프라인을 돌리는지 목록에서 한눈에 보이게 하기 위함이다. agents 세션이 아니면 조용히
no-op이고 절대 실패하지 않으므로 항상 호출한다. 이후 각 하위 단계 스킬이 자기 라벨로 제목을
갱신하면 그대로 둔다(진행 단계가 제목에 드러난다).
