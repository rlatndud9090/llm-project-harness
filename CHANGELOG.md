...
같은 결함이 반복 지적되던 두 근본 패턴을 리뷰 지침과 반영 프로토콜에 못 박았다.

**진단** — 53건에 오타는 없었고, 두 결함 클래스가 각각 열 건 넘게 반복 발현됐다: (1) 경로·파일명
식별자 검증이 blocklist라 코덱스가 위험 문자를 하나씩 찾아낼 때마다 그 문자만 추가하는 땜빵이
`-`→`/?#`→`\`→`.`/`..`→탭·LF·CR→Win32 `*:"<>|`→대소문자→후행 점·공백→NFC/NFD→예약 산출물명으로
15건 넘게 이어졌다. (2) 제목 유일성을 라벨 문자열 합성으로 만들어 "서로 다른 조합 → 서로 다른 제목"
계약이 구조적으로 깨졌다(9건). 두 경우 모두 "지적된 그 값만" 고쳐 push해 재리뷰가 N연속 재발했다.

**변경**

- `harness/guides/code-review-guideline.md`:
  - §0 코어 렌즈 3에 **allowlist 원칙** 추가 — 위험값을 하나씩 금지(blocklist)하면 다음 케이스가
    반드시 또 뚫린다; 안전 집합만 허용하거나 최종 산출물(경로·파일명·제목·키) 기준으로 완전성을
    검증하라. "이 문자도 막으세요"류 지적은 관문이 blocklist라는 신호다.
  - §10에 **경로/식별자 완전성 detector** 신설(URL 예약·FS 예약·빌드 예약·유니코드 정규형·합성
    바이트 길이·로케일 정렬 6축을 개별 문자가 아니라 산출물 기준으로 한 번에 닫는다), §2에 **합성
    유일성**·**파생 표시값 정합** detector, §3에 **env-as-URL 경계 검증**(+파싱 실패 메시지의 비밀
    노출), §10에 정적 산출물 canonical/og 완전성·테스트-빌드 산출물 의존·`.gitignore` 시크릿 규칙 회귀.
  - "후속 트래킹"에 **안티-땜빵 조항** — 열거형·반복 지적은 개별 케이스가 아니라 결함 클래스의
    입력 공간 전체를 닫는다(그 한 건만 고쳐 push하면 N연속 재발·재리뷰 토큰 소모).
  - 코퍼스 high-water: `pet-spec-finder #4`(53건) 추가, 총계 391→444(P1 57→60).
- `skills/pr-codex-once`·`pr-codex-loop`·`pr-self-loop`: 수정 단계의 "최소·정확한 수정"을
  **"개별 케이스가 아니라 결함 클래스를 닫는다(안티-땜빵)"**로 교체 — 봉인 테스트도 지적된 입력만이
  아니라 같은 클래스의 경계·조합까지 덮는다.

**소비자 조치: 없음.** 마켓플레이스 갱신(`/plugin marketplace update`)만으로 새 지침·스킬이
반영된다. 배선 변경 없음.
