...
에이전트는 독자 규칙을 만들지 않고 공용 하네스의 `$do-next` 흐름을 따른다.
...
- 의도와 결정 경계는 실행 환경에 설치된 `$deep-interview`로 좁힌다.
- 런타임의 structured question UI가 있으면 그 경로를 사용한다.
...
규칙 변경은 runtime adapter가 아니라 `.harness/harness`를 먼저 수정한다.
