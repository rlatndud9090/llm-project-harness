...
| `title` | 한국어 작업 제목 |
| `area` | 이 작업이 발전시키는 기능/구조 영역(기존 영역 재사용 우선, 없으면 신설, broad 금지). 여러 개면 콤마 |
| `why now` | 지금 해야 하는 이유 |
| `scope` | 이 작업에 포함할 것 |
...
4. 확정된 작업 단위를 `docs/raw/.next-unit`에 `<type>/<slug> | <한국어 제목> | <영역>`
   한 줄로 기록한다(영역이 여러 개면 콤마). 다음 `$kickoff`가 이 anchor를 읽어 resolved
   단위와 대조하고, 3번째 필드의 영역을 `--area` 없이도 frontmatter에 시드한 뒤 소비한다.
   채팅에만 남기면 kickoff가 다른 slug/영역으로 흘러도 무탐지로 통과하므로 anchor를 남긴다.
