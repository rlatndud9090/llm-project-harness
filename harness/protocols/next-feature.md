...
작업 트리가 깨끗하면 `$kickoff`이 작업 브랜치도 자동으로 만들어 전환한다. 다른 브랜치이거나
커밋 안 된 변경이 있으면 자동 전환하지 않고, 워크트리 격리 vs 현재 위치 checkout을 사용자에게
물어본다(자세한 규칙은 `kickoff.md`). PRD 작성은 그 다음 `$prd-helper`가 맡는다.
