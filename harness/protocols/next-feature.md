...
- 사용자 의도, 비목표, 결정 경계를 먼저 확정한다(`$deep-interview`가 있으면 활용, 없으면 명시 질문).
...
1. 기본값은 한 번에 하나의 명시 질문만 묻고 답을 기다리는 것이다.
2. `$deep-interview`가 설치돼 있으면 그걸로 구조화한다(선택). tmux에 붙은 OMX/OMC
   런타임이면 `omx question` 같은 런타임 구조화 질문을 쓸 수 있다.
