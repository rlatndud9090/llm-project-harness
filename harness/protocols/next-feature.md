...
- 사용자 의도, 비목표, 결정 경계를 먼저 확정한다(`$deep-interview`가 있으면 활용하고, 없으면 현재 런타임의 구조화 질문 도구를 우선 사용하며 그마저 없을 때만 명시 질문).
...
1. 기본값은 현재 런타임의 구조화 질문 도구를 우선 사용해 질문을 제시하고 답을
   받는 것이다. 구조화 질문 도구가 없을 때만 한 번에 하나의 간결한 명시 질문으로
   fallback한다.
2. `$deep-interview`가 설치돼 있고 현재 surface가 그 구조화 질문 surface를 지원하면
   그걸 가속기로 쓴다(선택). tmux에 붙은 OMX/OMC 런타임이면 `omx question` 같은
   런타임 구조화 질문을 쓸 수 있다.
...
4. 확정된 작업 단위를 `docs/raw/.next-unit`에 `<type>/<slug> | <한국어 제목>` 한 줄로
   기록한다. 다음 `$kickoff`가 이 anchor를 읽어 resolved 단위와 대조하고 소비한다.
   채팅에만 남기면 kickoff가 다른 slug로 흘러도 무탐지로 통과하므로 anchor를 남긴다.
