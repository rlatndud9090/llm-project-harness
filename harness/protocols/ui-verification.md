...
4. screenshot 또는 관찰 결과를 notes에 남긴다. 스크린샷·화면 캡처 파일을 **사용자에게 제시할 때는 raw 경로가 아니라 `[스크린샷 설명](절대경로)` 마크다운 링크(절대경로)로 준다**(`session-start.md` "사용자에게 로컬 경로·링크를 제시하는 규약"; raw 경로는 워크트리·일부 터미널에서 링크가 잘려 안 열린다).
