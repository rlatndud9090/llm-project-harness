...
## 위키 파일에 무엇을 두는가 (내용 계약)

소비 프로젝트의 `docs/wiki/index.md`(및 분리된 `docs/wiki/<섹션>.md`)에는 **그
프로젝트에 대한 내용만** 둔다. 구체적으로:

- **frontmatter**: `summary`·`authoring_rules` 같은 몇 줄의 설명(이 파일이 무엇이고 작성
  규칙은 어디에 있는지 가리키는 포인터). 규칙 본문을 여기에 복붙하지 않는다.
- **`## 큰 방향성`**: 이 프로젝트의 무엇/누구/목표/어떻게/지식 경계.
- **`## Raw Units (영역별 계보)`** 아래 `### <영역>`별 dated 계보 링크.

**이 문서(= `wiki-ingest.md`)에 있는 '위키 작성 규칙' 설명은 위키 파일에 넣지 않는다.**
"영역이 무엇인가", "타임라인 읽는 법", "ingest 실행법", "Maintenance 규칙" 같은 안내
문단·상단 blockquote·하단 Maintenance 섹션은 전부 여기(harness 프로토콜)에 있고, 위키
파일에는 남기지 않는다. 옛 템플릿/attach 시드는 이 규칙을 위키에 통째로 복사했는데,
`harness:check`(`assertWikiNoAuthoringGuidance`)가 그 잔재를 sentinel로 감지해 하드
에러로 막는다. 서브모듈을 올린 소비 프로젝트는 위키에서 규칙 문구를 삭제해야 통과한다
(방향성·계보 링크는 유지).
