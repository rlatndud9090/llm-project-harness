...
## `## 큰 방향성`도 함께 갱신한다 (필요할 때 · 의미 판단)

ingest 도구는 area **계보 링크**만 기계로 갱신한다. 하지만 `docs/wiki/index.md`의
`## 큰 방향성`(이 프로젝트의 무엇/누구/목표/어떻게/지식 경계)은 프로젝트가 진화하면 함께
낡는다 — 그런데 소비 프로젝트가 이 부분을 거의 손대지 않아 방향성이 "초기 스캐폴드" 상태로
정체되는 일이 잦다.

- **이 작업이 프로젝트의 큰 방향성을 실제로 바꿨다면**(새 목표·범위 확장/축소·지원 대상 변화·
  지식 경계 이동·핵심 접근법 전환), ingest와 **같은 변경**에서 `## 큰 방향성`도 갱신한다. 계보
  링크만 추가하고 방향성을 옛말로 두면, 다음 세션이 위키 첫 화면에서 잘못된 프로젝트 상을 읽는다.
- **언제 특히**: 둘째 touch(통합/커밋 직전, 결정이 확정된 시점). 이때 "이 결정이 방향성 문장
  하나라도 바꾸는가?"를 점검하고, 바꾼다면 그 문장을 고친다.
- **강제하지 않는다(의미 판단).** 방향성이 바뀌었는지는 모델·사람의 판단이므로 `harness:check`가
  하드 게이트로 막지 않는다. 대신 `harness:ingest`가 의미 있는 ingest 뒤에 한 줄로 리마인드한다.
  방향성이 안 바뀐 소소한 작업이면 그대로 두면 된다(무의미한 갱신 강요 아님).
- 큰 방향성은 **짧은 방향 서술**이다(요구/결정 상세가 아니다 — 그건 raw). 링크·계보는 아래
  Raw Units 계보가 담고, 이 섹션은 "지금 이 프로젝트가 무엇을 향하는가"만 신선하게 유지한다.
