...
나란히 **화면 배치 결정**도 포착한다. `designer` 역할로 진행한다:

1. `designer`가 화면 배치 대안을 **최소 2개** ASCII 와이어프레임으로 그리고, 각 대안의 정보
   우선순위·상호작용 어포던스·트레이드오프를 비교한다.
2. 배치 대안을 ADR `## 선택지`에(아키텍처 선택지와 같은 자리) 넣고, 채택안을 `## 결정`,
   시각 위계 근거와 기각 사유를 `## 선택 근거`에 남긴다. **별도 아티팩트를 만들지 않는다** —
   디자인 결정은 이 ADR에 산다.
3. 배치 선택은 사용자가 고른다: 현재 런타임의 구조화 질문 도구(ClaudeCode는 `AskUserQuestion`,
   ASCII 목업을 `preview`로 side-by-side 제시)로 대안을 보여주고 선택을 받는다. 그 발화는
   아래 PRD·ADR **통합 승인**으로 흡수된다(별도 승인 경로 없음).

기본 산출물은 tool-free ASCII다(Codex/ClaudeCode 양쪽 동작). 아래는 **있으면 쓰는 optional
가속기**이며, 없어도 ASCII로 진행한다:

- `frontend-design` 스킬 — 미적 방향·타이포·"템플릿 같지 않은" 판단 보조.
- 브라우저 도구 — 대안별 정적 목업 HTML을 렌더해 스크린샷으로 hi-fi 비교. **토큰 효율이 가장 좋은 인터페이스를 최우선**한다: 부분 스냅샷·스크린샷만 되받는 CLI 드라이버(예: Playwright CLI; depth 제한·부분 스냅샷·raw 스트립)가 기본값, 매 단계 전체 접근성 스냅샷을 되돌리는 브라우저 제어 MCP는 CLI로 도저히 안 될 때만.
- 컴포넌트 라이브러리 MCP — 소비 프로젝트가 특정 UI 킷을 쓰면 실제 컴포넌트로 현실적 배치 제안.
