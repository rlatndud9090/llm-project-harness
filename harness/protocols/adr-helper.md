...
1. **대안 최소 2개**를 화면 배치로 설계하고, 각 대안의 정보 우선순위·상호작용 어포던스·
   트레이드오프를 정리한다.
2. **비교 HTML 파일을 생성한다 — 템플릿을 복사한 뒤 채우기 구역만 편집한다(스캐폴딩 재출력 금지).**
   이 파일은 이미 fill-zone이 표시된 템플릿이다. 파일 전체를 `Write`로 다시 쓰면 CSS·JS·패널·
   사용법 등 형식이 고정된 ~9KB 보일러플레이트를 매번 **출력 토큰으로 재생성**하게 되므로,
   반드시 **복사 후 목업만 주입**한다:
   - ① 템플릿을 목적지로 **그대로 복사**한다:
     `cp "${CLAUDE_PLUGIN_ROOT}/harness/templates/design/design-options.html" docs/raw/<type>/<slug>/design-options.html`
   - ② 복사본에서 **채우기 구역만 `Edit`**으로 고친다: `{제목}`(title·h1 2곳), 각 대안 카드의
     `data-label`·라디오 `value`·`.name`(대안 이름으로 함께 일치), `.mockup` 내부(실제 배치 목업 —
     HTML/CSS, 레이아웃·위계 중심의 중립 톤). 대안 수(2개 이상)는 카드 `<section class="option">`
     블록을 복제/삭제해 맞춘다.
   - ③ **CSS·JS·패널·사용법 스캐폴딩은 절대 다시 출력하지 않는다.** 목업(진짜 작업물)만 출력 토큰을 쓴다.

   **Claude 아티팩트를 쓰지 않는다** — 반드시 로컬 파일로 떨군다(자기완결·외부 참조 없음이라
   사용자가 브라우저에서 바로 연다).
3. **사용자에게 파일 경로를 마크다운 링크로 알리고 열게 한다** — raw 경로가 아니라
   `[디자인 비교 화면](<design-options.html 절대경로>)` 형식(절대경로)으로 준다(`session-start.md`
   "사용자에게 로컬 경로·링크를 제시하는 규약"; raw 경로는 워크트리·일부 터미널에서 링크가 잘려 안 열린다).
   사용자는 대안을 나란히 비교 → 베이스 라디오
