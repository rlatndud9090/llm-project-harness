# PRD Helper 프로토콜

`$prd-helper`는 PRD를 함께 작성하는 보조 단계다. PRD는 구현 지시서가 아니라
"왜 이 작업을 하는가, 무엇이 완료인가, 무엇은 하지 않는가"를 보존하는 제품
아티팩트다. 이 단계는 세 흐름을 엮어 PRD 품질을 끌어올린다.

- `$deep-interview`로 사용자 의견과 결정 경계를 수집한다.
- `researcher` 역할로 레퍼런스/선행 사례/참고 자료를 수집한다.
- `reviewer` 역할로 작성 중인 PRD를 지속 검토한다.

전제: `$kickoff`로 raw unit(`docs/raw/<type>/<slug>/`)이 이미 생성돼 있고, 그 안에
단계 체크포인트 원장 `state.md`가 있다. PRD는 `review` 상태로 작성한다. 승인은 2단계이며,
이 단계에서 받는 것은 **사전 승인(pre-approval)** — 빌드(feature-develop) 진입 게이트 — 이다:
사용자 명시 사전 승인 전에는 PRD를 `pre-approved`로 바꾸지 않는다. `pre-approved` 전환은 오직
`harness:approve`로만 한다(직접 frontmatter 편집 금지 — 런타임 훅과 `harness:check`가 막는다).
최종 `approved` 확정은 이 단계가 아니라 `$make-pr`(PR 직전)에서 이뤄진다.

## 이 단계의 범위 (엄격한 단계 분리)

**`$prd-helper`는 `prd.md`만 편집한다. `adr.md`는 절대 건드리지 않는다.** `$kickoff`가
`adr.md` 스켈레톤을 미리 만들어 두더라도 이 단계에서는 스켈레톤 그대로 둔다. ADR을
작성하는 것은 다음 단계인 `$adr-helper`의 몫이며, `adr.md` 작성은 `state.md`의 `stage`가
`adr-draft` 이상으로 올라간 뒤에만 허용된다(그 전에는 런타임 가드와 `harness:check`가
막는다). ADR이 필요한지 여부와 그 이유는 `adr.md`가 아니라 **PRD의 `## ADR 필요 여부`
섹션**에 기록한다.

이 단계의 산출물은 오직 두 갈래로만 끝난다:

- **ADR 불필요**로 판정 → PRD 단독 **사전 승인**을 대기한다(아래 "정리와 사전 승인").
- **ADR 필요**로 판정 → 사전 승인을 아직 요청하지 않고 `$adr-helper`로 넘긴다. 사전 승인은
  ADR까지 작성된 뒤 PRD·ADR을 **한 번에** 받는다(`$adr-helper`가 담당).

## 언어

PRD는 한국어로 작성한다. branch name, file path, command, TypeScript identifier처럼
기계가 읽거나 코드와 직접 연결되는 표현만 영어를 유지한다.

## 진행 루프

단순한 PRD(작은 범위·명확한 요구)는 메인이 직접 작성·검토한다. `researcher`/`prd-writer`/`reviewer`
서브에이전트 위임은 리서치·리뷰 부담이 실제로 클 때만 편다(위임 비용은 `session-start.md` "위임 비용" 참조).

### Phase 1: 의견 수집 (deep-interview 진입은 판단해서 — 강제 기본값 아님)

의견 수집을 시작하기 전에 **deep-interview로 들어갈지를 먼저 판단한다.** deep-interview는 강제
기본값이 아니라, 아래 판단이 "모호/큰 결정"을 가리킬 때 꺼내 쓰는 도구다. 판단은 두 가지를
가늠해서 한다:

1. **이번 작업의 범위·깊이** — 한 화면·한 규칙의 좁은 작업인가, 여러 제품 결정·경계가 걸린 큰
   작업인가.
2. **kickoff에서 받은 사용자 컨텍스트만으로 이미 명확한가** — 그 컨텍스트를 그대로 PRD 수용 기준으로
   옮겨도 통과/실패를 판정할 수 있으면 이미 명확한 것이고, 가정을 여러 곳 메워야 하면 모호한 것이다.

판단에 따라 경로를 고른다:

- **deep-interview로 진입** (설치돼 있을 때): kickoff 컨텍스트만으로는 목표·제약·성공 기준에 실제
  모호함이 남거나, 범위가 크거나 여러 제품 결정·숨은 가정이 걸린 작업일 때. 그 스킬을 실제로 호출해
  ambiguity를 수치로 측정하며 가장 약한 차원을 좁힌다(threshold까지). crystallize한 spec의
  Goal/Constraints/Non-Goals/Acceptance Criteria/노출 가정을 각각 PRD의 `## 목표`/`## 비목표`/
  `## 요구사항`/`## 수용 기준`/`## 열린 질문`으로 옮긴다(고유명사·구현 누수는 PRD 고도 루브릭대로 소거).
  **실행 브리지(omc-plan/autopilot/ralph/team 핸드오프)는 타지 않는다** — 하네스 흐름(prd → adr →
  feature-develop)을 유지하며 인터뷰·crystallize까지만 쓴다.
- **바로 진행 / 단발 질문**: kickoff 컨텍스트가 이미 목표·범위·기준을 구체적으로 담고 있거나 범위가
  좁아 ambiguity가 낮으면 deep-interview를 강행하지 않는다. 남은 몇 가지만 현재 런타임의 구조화 질문
  도구(ClaudeCode는 `AskUserQuestion`)로 확인하고 PRD를 쓴다.
- **경계에서 애매하면**: 어느 쪽인지 판단이 서지 않으면 사용자에게 한 줄로 물어 정한다 — "요구를 더
  파고들까요(deep-interview) / 이대로 진행할까요?".

어느 경로든 사용자가 직접 판단해야 하는 항목은 숨은 가정으로 처리하지 않고 먼저 묻는다.
`$deep-interview`가 없는 런타임(Codex 등)에서 진입이 필요하다고 판단되면 구조화 질문 도구로, 그것도
없으면 한 번에 하나씩 명시 질문으로 진행하되, "한 라운드로 끝내지 않고 가장 약한 지점을 좁혀 간다"는
규율은 같다.

### Phase 2: 레퍼런스 수집 (`researcher`)

1. `researcher` 역할로 유사 제품, 선행 사례, 표준, 제약을 조사한다.
2. 조사 결과는 출처와 함께 `notes.md`에 정리하고, PRD에는 결론만 반영한다.
3. 조사가 PRD의 어떤 요구/비목표/수용 기준을 뒷받침하는지 연결한다.

### Phase 3: 초안 작성

아래 필수 섹션으로 PRD를 작성한다.

```md
## 배경
## 목표
## 비목표
## 요구사항
### 기능 요구사항
### 비기능 요구사항
## 수용 기준
## 열린 질문
## ADR 필요 여부
## 관련 문서
```

작성 기준:

- 배경은 사용자 또는 프로젝트 관점의 긴장을 설명한다.
- 목표는 이 작업이 달성해야 하는 결과를 적는다.
- 비목표는 범위 팽창을 막기 위해 구체적으로 적는다.
- 기능 요구사항은 사용자가 경험하거나 개발자가 구현해야 하는 동작을 적는다.
- 비기능 요구사항은 성능, 접근성, 반응형, 저장소 구조, 검증 가능성 같은 품질 조건을 적는다.
- 수용 기준은 검증자가 통과/실패를 판단할 수 있어야 한다.
- 열린 질문은 숨은 가정으로 묻지 말고 명시한다.
- **영역(area)**: 이 작업이 발전시키는 기능/구조 단위를 판단해 `prd.md` frontmatter
  `area:`에 적는다(review로 올리기 전에). **먼저 `docs/wiki/index.md`의 `### 헤딩` 목록을
  읽어 기존 영역을 확인하고**, 이 작업이 그 중 하나의 연속이면 그 이름을 **오타 없이 그대로**
  재사용한다(wiki `### 헤딩`과 정확히 일치해야 하며, `A화면`≠`A 화면`처럼 어긋나면 영역이
  갈라진다). 없으면 구체적인 새 영역을 만든다. broad 바구니는 금지. 소규모의 다른 영역
  작업이 섞였으면 콤마로 여러 영역을 적는다. 이건 의미 판단이므로 `$prd-helper`가 정하며,
  wiki 그룹핑·타임라인은 이 값에서 파생된다.
- **섹션(section, 선택)**: 프로젝트가 area 상위의 **섹션**(웹앱 최상위 라우팅/제품 영역
  단위)을 쓰면, 이 작업이 어느 섹션인지 판단해 `prd.md` frontmatter `section:`에 **단일
  값**으로 적는다. 이미 분리된(섹션 2개 이상) 프로젝트라면 필수다 — 미선언이면 ingest가
  실패한다. 기존 섹션은 `docs/wiki/index.md`의 `## 섹션` 허브(또는 각 섹션 파일)에서 확인해
  연속이면 오타 없이 재사용한다. 섹션이 2개째로 늘면 `harness:ingest`가 wiki를 섹션별 파일로
  자동 분리한다. 섹션을 안 쓰는 프로젝트면 비워 둔다.
- **상위 PRD 계보(선택)**: 이 PRD가 상위/부모 PRD(큰 계약)를 세부화하는 후속 작업이면
  `prd.md` frontmatter `parent_prd`에 그 상대경로(예: `../<상위-slug>/prd.md`)를 적는다.
  `harness:check`가 링크 유효성을 검증한다. 계보가 없으면 비워 둔다.

### Phase 4: 지속 리뷰 (`reviewer`)

`reviewer` 역할로 초안을 반복 검토하고, 지적 사항을 반영해 다시 다듬는다.

- 수용 기준이 관찰 가능한가? (통과/실패를 판정할 수 있는가)
- 목표와 비목표가 서로 모순되지 않는가?
- 누락된 비기능 요구(접근성/반응형/저장소/검증)는 없는가?
- **구현 세부가 요구에 새지 않았는가?** 각 요구/수용 기준에 고도 self-check를 적용한다:
  1. **실제 요구 판정** — "무엇이 만족돼야 하는가"인가, 아니면 "어떻게 만들지"(설계/구현)인가?
  2. **구현 결합 판정** — 구현 방식이 바뀌면 이 문장도 바꿔야 하는가? 그렇다면 새어든 것이다.
  3. **고유명사 소거 판정** — 특정 언어·라이브러리·파일명·타입명·필드명을 지우면 요구가
     무너지는가? 무너지면 그 고유명사가 구현 누수다(에이전트가 코드베이스를 읽은 뒤 특히 잘
     저지른다). 지워도 남는 "무엇/왜"만 PRD에 남기고, 그게 다투는 결정이면 ADR로, 아니면 구현으로.
- **구현을 빼려다 모호해지지 않았는가?** 과소명세도 동등한 실패다. "구현 중립적이되 관찰·통과/
  실패 판정 가능"이 목표다.
- **개인 호칭·별명이 PRD에 새지 않았는가?** 사람은 중립 역할어(`사용자`/`개발자`/`검증자`)로만
  지칭한다. 런타임 개인 설정이 사용자를 특정 호칭(예: `형님`)으로 부르라 해도 그 페르소나는
  대화에만 두고 산출물에는 넣지 않는다(단일 출처: `templates/examples/README.md` §5).
- 레퍼런스 결론이 요구사항과 실제로 연결되는가?

리뷰 → 수정 루프는 위 질문에 **모두 답할 수 있을 때 종료한다**(무한 재작성 방지). 전체 고도
루브릭·누수 부검 대조표·채워진 짝 예시는 `templates/examples/README.md`를 **단일 출처**로 참고한다
(내용을 이 프로토콜에 복제하지 않는다).

### Phase 5: ADR 필요 여부 판단 (여기서 판단만 하고 ADR은 쓰지 않는다)

PRD의 `## ADR 필요 여부` 섹션에서 구조/데이터/엔진/의존성 같은 durable decision이
필요한지 판단하고, **그 결론과 이유를 이 PRD 섹션에 적는다.** 이 판단은 `$prd-helper`가
하지만, ADR 본문 작성은 여기서 하지 않는다(`adr.md`를 편집하지 않는다).

durable decision에는 **화면 배치 결정**도 포함될 수 있다. 이 유닛이 UI-significant이고
화면 구성이 실제 다투는 결정이면(대안이 여럿, 되돌리기 비용, "왜 이 배치"라는 근거를
남길 가치 — 예: 선출 서수·배지 위치, 세로 배치 순서, 스티키 버튼, 핵심 정보 위계) 그것도
"ADR 필요"로 본다. 이때 ADR 필요 여부에 그 사유를 함께 적고, `$adr-helper`의 **디자인 결정
레인**(`designer` 역할)에서 배치 대안을 비교·결정한다. 단순 화면(다투는 배치가 아님)은
디자인 결정 레인을 태우지 않고 build-first로 둔다.

- **필요하면**(구조/데이터/엔진/의존성 또는 다투는 화면 배치): PRD의 `## ADR 필요 여부`에
  "필요: <이유>"를 적고, **여기서 멈춘다.** PRD 사전 승인을 요청하지 말고 `state.md`의 `stage`를
  `prd-review`로 둔 채 다음 스킬로 `$adr-helper`를 안내한다. `adr.md`는 스켈레톤 그대로 둔다.
- **필요 없으면**: PRD의 `## ADR 필요 여부`에 "불필요: <이유>"를 적는다. `adr.md`는
  손대지 않고 스켈레톤(status `proposed`)으로 그대로 둔다. 그다음 아래 "정리와 사전 승인"의
  **PRD 단독 사전 승인**으로 진행한다.

`adr.md` 파일 자체는 삭제하지 않는다. feature 단위는 항상 `adr.md`를 유지한다(`$kickoff`가
생성하고 `harness:check`가 존재를 요구한다). ADR이 불필요해도 그 사유는 `adr.md`가 아니라
PRD의 `## ADR 필요 여부`에 남기고, `adr.md`는 미작성 스켈레톤으로 둔다.

## 정리와 사전 승인

PRD 초안이 자리를 잡으면(area가 frontmatter에 선언된 상태에서):

```sh
npm run harness:ingest -- docs/raw/<type>/<slug>
npm run harness:check
```

이 `harness:ingest`가 wiki lineage의 **첫 touch**다(선언한 area 아래 dated 링크 생성 →
`harness:check`의 "모든 raw unit은 링크돼야 한다" 통과). `_(현재)_`/superseded 큐레이션은
통합/커밋 시점의 둘째 touch에서 한다(`wiki-ingest.md` "실행 시점" 참고).

이 단계는 PRD를 `review` 상태로 마무리한다. `review`로 올릴 때 `state.md`의 `stage`를
`prd-review`로, `prd_status`를 `review`로 갱신하고 단계 로그에 한 줄 남긴다. 여기까지가
이 프로토콜의 끝이다. **여기서 최종 `approved`로 넘어가지 않는다**(최종 확정은 `$make-pr`).
이 단계의 승인은 `pre-approved`(사전 승인)까지다.

### 사전 승인 분기: ADR 필요 여부에 따라 갈린다

- **ADR 필요로 판정한 경우 — 여기서 사전 승인을 요청하지 않는다.** PRD는 `review`,
  `state.md`는 `prd-review`로 둔 채 `$adr-helper`로 넘어간다. PRD·ADR 통합 사전 승인은
  `$adr-helper`가 담당한다(ADR 피드백을 반영하다 PRD를 함께 고쳐야 할 수 있으므로 한
  번에 사전 승인한다).
- **ADR 불필요로 판정한 경우에만 — 여기서 PRD 단독 사전 승인을 요청한다.** 아래 명시 의례를
  따른다.

### PRD 단독 사전 승인은 오직 명시 의례로만 (ADR 불필요일 때)

`pre-approved` 전환은 사용자의 명시 사전 승인이 있을 때만, 오직 `harness:approve`로만 한다.

1. 현재 런타임의 구조화 질문 도구(ClaudeCode는 `AskUserQuestion`)로, **대상 문서와 전환
   상태를 명시해** 사전 승인을 요청한다. 예: "이 PRD를 pre-approved로 올려 구현에 들어갈까요?
   (사전 승인 / 아직)". 이건 잠정 승인이며, 구현 결과를 보고 PRD를 개정할 수 있고 최종 확정은
   `$make-pr`에서 다시 받는다는 점을 함께 알린다.
2. 사용자가 분명히 사전 승인하면 **그 발화를 그대로 인용**해 실행한다.

```sh
npm run harness:approve -- --unit docs/raw/<type>/<slug> --quote "<사용자의 사전 승인 발화 verbatim>"
```

`harness:approve`(기본 모드)가 status를 `pre-approved`로, `approval:` 근거와 `state.md`
`PREAPPROVAL` 이벤트를 원자적으로 함께 기록한다(`--adr` 없이 PRD만 전환한다). 에이전트가 직접
frontmatter status를 고치지 않는다.

**사전 승인으로 간주하지 않는 것(중요):** "이렇게 하려고 했어", "이거 작업해야 돼", "좋아
보인다", 목표·범위·아이디어 설명, 인터뷰 질문에 대한 답 — 이 중 어느 것도 사전 승인이
아니다. 사전 승인은 위 사전 승인 요청에 대한 사용자의 분명한 긍정 응답만을 뜻한다. 조금이라도
모호하면 사전 승인이 아니며, `review`로 둔 채 다시 구조화 질문으로 확인한다.

구현은 사전 승인(`PREAPPROVAL`)이 `state.md`에 기록된 이후 `feature-develop`에서 진행한다.

## 품질 게이트

PRD 초안은 아래 질문에 답할 수 있어야 한다.

- 왜 지금 이 작업인가?
- 이 작업이 끝났다고 어떻게 판단하는가?
- 어떤 일은 일부러 하지 않는가?
- 다음 작업 단위와 어떻게 연결되는가?
- 구현자가 설계 결정을 별도로 ADR에 남겨야 하는가?

## 실패 모드

- **나쁨:** "데이터를 만든다"처럼 선언만 있고 수용 기준이 없다.
- **좋음:** "필수 참조 데이터가 중복 없이 로드되고 누락 시 검증이 실패한다"처럼 관찰 가능한 기준을 둔다.

- **나쁨:** 레퍼런스 조사 없이 감으로 요구사항을 적는다.
- **좋음:** `researcher`로 선행 사례를 모으고 결론을 요구사항에 연결한다.

- **나쁨:** 초안을 한 번 쓰고 그대로 둔다.
- **좋음:** `reviewer` 리뷰 → 수정 루프로 수용 기준과 비목표를 다듬는다.

- **나쁨:** PRD 초안을 작성하자마자 `pre-approved`(또는 `approved`)로 둔다.
- **좋음:** `review`로 두고, 사용자 명시 사전 승인 발화를 받아 `harness:approve`로 `pre-approved`로 바꾼다.

- **나쁨:** "이렇게 하려고 했어" 같은 의도·아이디어 발화를 사전 승인으로 오인해 바로 전환·구현한다.
- **좋음:** 그런 발화는 사전 승인이 아니라고 보고, 구조화 질문으로 명시 사전 승인을 다시 확인한다.

- **나쁨:** PRD를 쓰다 같은 흐름으로 `adr.md`까지 채워 넣는다(단계가 뒤섞인다).
- **좋음:** `adr.md`는 손대지 않고, ADR 필요 여부·이유만 PRD `## ADR 필요 여부`에 남긴 뒤
  필요하면 `$adr-helper`로 넘긴다. `adr.md` 작성은 `stage`가 `adr-draft`로 올라간 다음이다.

- **나쁨:** ADR이 필요한데 PRD만 먼저 사전 승인받고 `$adr-helper`로 넘어간다.
- **좋음:** ADR이 필요하면 PRD 사전 승인을 미루고, ADR까지 작성한 뒤 PRD·ADR을 한 번에 사전 승인받는다.
