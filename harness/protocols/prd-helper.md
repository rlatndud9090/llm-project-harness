...
1. 목표, 성공 기준, 비목표, 결정 경계를 끌어낸다. 현재 런타임의 구조화 질문
   도구를 우선 사용하고, `$deep-interview`가 설치돼 있으며 현재 surface가 그
   구조화 질문 surface를 지원하면 가속기로 쓴다. 구조화 질문 도구가 없을 때만
   한 번에 하나씩 간결한 명시 질문으로 fallback한다.
2. 사용자가 직접 판단해야 하는 항목은 숨은 가정으로 처리하지 않고 구조화 질문
   도구로 먼저 묻는다. 그 도구가 없을 때만 명시 질문한다.
