...
>
> **섹션(section)과 분리:** area보다 큰 상위 단위가 **섹션**이다. 계층은
> `## 섹션 > ### 영역(area) > 시간순 bullet`. 프로젝트에 선언된 섹션이 1개 이하인
> 동안에는 이 index.md 한 장에 모든 영역을 담는다. 섹션이 **2개 이상**이 되는 순간
> `harness:ingest`가 각 섹션을 `docs/wiki/<섹션>.md`로 **자동 분리**하고, 이 index.md는
> 각 섹션 파일로 가는 **링크 허브**가 된다(첫 섹션의 기존 영역 계보도 도구가 함께
> 옮긴다). 섹션은 각 unit의 `prd.md`/`bugfix.md` frontmatter `section:`에 선언한다.
...
- 섹션(area 상위 단위)이 1개 이하면 wiki는 이 index 한 장을 유지한다. 섹션이 2개 이상이 되면 `harness:ingest`가 `docs/wiki/<섹션>.md`로 자동 분리하고 이 index는 섹션 링크 허브가 된다.
- 상세 설명은 wiki가 아니라 raw 문서에 남긴다.
