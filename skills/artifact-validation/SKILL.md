...
2. `${CLAUDE_PLUGIN_ROOT}/harness/protocols/artifact-validation.md`를 읽는다.
3. `node "${CLAUDE_PLUGIN_ROOT}/scripts/harness/artifact-check.mjs"`를 실행한다.
...
`node "${CLAUDE_PLUGIN_ROOT}/scripts/harness/approve.mjs"`로만 한다.
