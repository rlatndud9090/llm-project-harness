...
- 이 스킬의 핸드오프 질문: PR 생성 뒤 "PR 생성 완료 — 리뷰 루프(`$pr-codex-loop`)를 돌릴까요?"처럼 다음 스킬과 효과를 명시하고 "지금 진행 / 나중에" 선택지를 준다. **`$make-pr` 호출 자체가 명시 최종 승인이라 최종 확정은 다시 묻지 않는다** — 여기서 `AskUserQuestion`으로 끝내는 건 오직 PR 생성 뒤 리뷰 루프 핸드오프뿐이다.
