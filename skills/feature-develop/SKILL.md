...
훅은 `prd.md`/`adr.md`의 `status`를 `pre-approved`/`approved`/`pre-accepted`/`accepted`로
바꾸거나 `state.md`의 `prd_status`/`adr_status`를 승인축으로 바꾸는(또는 `- PREAPPROVAL`/
`- APPROVAL` 줄을 직접 넣는) 편집을 편집 결과 기준으로 exit 2 차단한다(`bypassPermissions`
