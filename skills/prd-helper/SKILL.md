...
- 이 스킬의 핸드오프 질문: "PRD를 review로 작성 완료 — ADR이 필요하면 지금 `$adr-helper`로, 불필요하면 이 PRD를 pre-approved로 올릴까요?"처럼 다음 스킬과 효과를 명시하고 선택지를 준다. **특히 ADR 필요로 `$adr-helper`에 넘길 때 "완료" 산문으로 끝내지 않는다** — 이 핸드오프가 대표적으로 알림을 놓치던 지점이다.
