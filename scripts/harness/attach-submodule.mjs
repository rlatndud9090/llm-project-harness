...
// The harness ships as a devDependency; `.harness` is a symlink into the installed
// `node_modules/llm-project-harness`, not a git submodule. Create it first so a
// first-time attach run straight from node_modules can use `.harness` as the mount —
// keeping adapter links and every `.harness/...` reference stable. Best-effort: a
// package-absent result (running from a linked `.harness` already, or the tests'
// fixture) just leaves the explicit --harness-dir / auto-detection in charge.
const linkResult = ensureHarnessLink(projectRoot, { dryRun });

// Default the mount to `.harness` once it resolves (the common case: a symlink into
// the installed package). Fall back to locating the harness from this script's own
// path when `.harness` is not present (e.g. dry-run before the first link).
const defaultHarnessDir = exists(path.join(projectRoot, ".harness", "harness", "protocols"))
  ? ".harness"
  : autoHarnessRoot(scriptPath);
const harnessRoot = path.resolve(projectRoot, args["harness-dir"] ?? defaultHarnessDir);
...
function symlinkRemediation(symlinksEntry) {
  ...
  steps.push(
    "하네스 devDependency를 설치한다(postinstall이 .harness 링크를 만든다): npm ci",
    "텍스트 파일로 체크아웃된 어댑터를 지우고 attach로 다시 링크한다(작업 트리가 깨끗할 때만): git ls-files -s | awk '$1 == 120000 { print $4 }' | xargs -r rm -f && node node_modules/llm-project-harness/scripts/harness/attach-submodule.mjs",
  );
...
function ensurePackageScripts() {
  ...
  changed = ensurePostinstallScript(packageJson) || changed;
...
// Wires the consumer `postinstall` to rebuild the `.harness` symlink after every
// install (link.mjs). `|| true` keeps a production `npm ci --omit=dev` — where the
// harness is absent — from failing the install. Never clobbers a project-owned
// postinstall: an existing one that already calls link.mjs is left alone, and any
// other is kept with a warning (re-run attach, or chain the command manually).
function ensurePostinstallScript(packageJson) {
  const command = "node node_modules/llm-project-harness/scripts/harness/link.mjs || true";
  const current = packageJson.scripts.postinstall;

  if (current === command) {
    operations.push("kept package script postinstall");
    return false;
  }
  if (typeof current === "string" && current.includes("link.mjs")) {
    operations.push(`kept package script postinstall: ${current}`);
    return false;
  }
  if (typeof current === "string" && current.trim() && !force) {
    operations.push(`kept package script postinstall: ${current}`);
    conflicts.push("package script override: postinstall");
    warnings.push(
      "기존 postinstall이 있어 하네스 링크를 자동으로 추가하지 못했습니다. " +
        "`node node_modules/llm-project-harness/scripts/harness/link.mjs || true`를 postinstall에 직접 이어 붙이거나, 설치 후 attach를 다시 실행하세요.",
    );
    return false;
  }

  packageJson.scripts.postinstall = command;
  operations.push("set package script postinstall");
  return true;
}

// Ensures the consumer `.gitignore` ignores the `.harness` mount (now a symlink into
// node_modules, regenerated by postinstall — never committed) and node_modules
// itself. Non-destructive: an existing `.gitignore` is appended to only for the
// entries it lacks; a project that already ignores them is left untouched.
function ensureGitignore() {
  const gitignorePath = path.join(projectRoot, ".gitignore");
  const wanted = [".harness", "node_modules"];

  if (!exists(gitignorePath)) {
    ensureFile(gitignorePath, `${wanted.join("\n")}\n`);
    return;
  }
  ...
  if (missing.length === 0) {
    operations.push(`kept ${relative(gitignorePath)}`);
    return;
  }

  operations.push(`append ${missing.join(", ")} to ${relative(gitignorePath)}`);
  if (dryRun) return;
  const prefix = current.endsWith("\n") || current.length === 0 ? "" : "\n";
  fs.writeFileSync(gitignorePath, `${current}${prefix}${missing.join("\n")}\n`, "utf8");
}

// Reports the outcome of the pre-flight `.harness` link step (see ensureHarnessLink).
// A created/would-create link becomes an operation line; an "occupied" `.harness`
// (most likely a not-yet-removed legacy git submodule) becomes a blocking-style
// warning so migration removes the submodule first. "ok"/"package-absent" are silent.
function recordHarnessLink(result) {
  if (!result) return;
  if (result.status === "created") {
    operations.push("link .harness -> node_modules/llm-project-harness");
  } else if (result.status === "would-create") {
    operations.push("link .harness -> node_modules/llm-project-harness (would create)");
  } else if (result.status === "occupied") {
    warnings.push(
      ".harness가 심볼릭 링크가 아니라 실제 디렉터리입니다(옛 git submodule일 수 있음). " +
